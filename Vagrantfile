...
Vagrant.configure(2) do |config|
  ...
  (1..$num_instances).each do |i|
    config.vm.define vm_name = format('%s-worker-%02d%s', $instance_name_prefix, i, $custom_networking_dnsDomain) do |node|
      ...
      config.dnsmasq.ip = proc do |guest_machine| 
        guest_machine.communicate.sudo("ifconfig enp0s | grep 'inet addr' | cut -d ':' -f 2 | cut -d ' ' -f 1") do |type, data| 
  ...
  # MASTER:
  config.vm.define vm_name = format('%s-master-01%s', $instance_name_prefix, $custom_networking_dnsDomain), primary: true do |k8smaster|
